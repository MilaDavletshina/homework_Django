...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        prod, _ = Product.objects.get_or_create(
            ...
            description="Беспроводной пылесос. Наслаждайся уборкой",
            category="Пылесос",
            price=11200,
        ...
        for cat_data in categories:
            ...
            if created:
                self.stdout.write(
                    self.style.SUCCESS(f"Successfully added: {category.name} ")
                )
            else:
                self.stdout.write(
                    self.style.SUCCESS(f"already exist: {category.name} ")
                )
